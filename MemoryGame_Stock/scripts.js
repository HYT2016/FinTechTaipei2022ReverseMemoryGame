...
function decideFrontFaceImg() {
  for (let i=1; i<selectCardImages.length; ++i) { //0905 0->1, i<s... => i<=
    frontFaces[2*i].src = './img/bond/' + selectCardImages[i-1]; //0905 i->i-1
    frontFaces[2*i+1].src = './img/bondAns/'+ selectCardImages[i-1];
  }
  frontFaces[1].src  = './img/bond/' + selectCardImages[selectCardImages.length-1];
  frontFaces[10].src = './img/bondAns/' + selectCardImages[selectCardImages.length-1];
}
// 0822 add
